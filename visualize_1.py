...
作者：didi
日期：2023年03月14日
内容：Functionality around spatial plotting.
"""
...
def draw_legend(im_ax, bbox=(1.05, 1), titles=None, cmap=None, classes=None):
    ...
    if classes is not None:
        ...
        colors = make_col_list(nclasses=len(classes), unique_vals=classes, cmap=cmap)
        ...
        if len(set(colors)) < len(classes):
            raise ValueError(
                ...
                "Please provide a ListedColormap with the same number of colors as classes."
            )
    ...
    if not len(classes) == len(titles):
        raise ValueError(
            ...
            "titles. You have provided {0} classes and {1} titles.".format(len(classes), len(titles))
        )
...
if __name__ == '__main__':
    '''
    色带参考：https://blog.csdn.net/qq_43874102/article/details/124363278?spm=1001.2014.3001.5501
    '''

    # 文件目录
    root_path = r"AQ600_data"
    out_path = r"AQ600_data/out"
    ...
    for filepath, dirnames, filenames in os.walk(root_path):
        for filename in filenames:
            if filename.endswith(".tif"):
                ...
                # 将图像的背景值设置为nan
                # raster_arr[raster_arr < -16] = 0  # np.nan, float('nan'), "nan"
                ...
                # 栅格数据可视化
                output_file_path_im = os.path.join(out_path, f"{filename.split('.')[0]}_T.jpg")
                plot_title = f"{filename.split('.')[0]}_LST"
                # plot_title = [f"{filename.split('.')[0]}_1",
                #               f"{filename.split('.')[0]}_2",
                #               f"{filename.split('.')[0]}_3",
                #               f"{filename.split('.')[0]}_4",
                #               f"{filename.split('.')[0]}_5",
                #               ]

                plot_bands(raster_arr,
                           title=plot_title,
                           title_set=[25, 'bold'],
                           cmap="cool",  # "seismic"
                           cols=3,
